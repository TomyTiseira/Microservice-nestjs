...
export class OrdersService extends PrismaClient implements OnModuleInit {
  ...
  async create(createOrderDto: CreateOrderDto) {
    try {
      ...
      const orderState = await this.statusService.getStatusByName(
        OrderStates.PENDING,
      );
  ...
  async findAll(orderPaginationDto: OrderPaginationDto) {
    ...
    const status = await this.statusService.getStatusByName(
      orderPaginationDto.status,
    );
  ...
  async changeOrderStatus(changeOrderStatusDto: changeOrderStatusDto) {
    ...
    const newStatusRecord = await this.statusService.getStatusByName(
      newStatus.getName(),
    );
